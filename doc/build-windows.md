...
Below are some notes on how to build tAustraliaCash Core for Windows.
...
Note that for WSL v1 the tAustraliaCash Core source path MUST be somewhere in the default mount file system, for
example /usr/src/taustraliacash, AND not under, for example, /mnt/d/taustraliacash. 
...
way. This will install to `c:\workspace\taustraliacash`, for example:
    ...
    make install DESTDIR=/mnt/c/workspace/taustraliacash
