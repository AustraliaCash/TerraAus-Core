# tAustraliaCash Core file system
...
The data directory is the default location where the tAustraliaCash Core files are stored.
...
3. All content of the data directory, except for the `taustraliacash.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`./`               | `taustraliacash.conf`       | User-defined configuration settings for `taustraliacashd` or `taustraliacash-qt`; can be specified by `-conf` option
`./`               | `taustraliacashd.pid`        | Stores the process ID (PID) of `taustraliacashd` or `taustraliacash-qt` while running; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `taustraliacashd` or `taustraliacash-qt`
...
`taustraliacash-qt` uses the [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by tAustraliaCash Core:
